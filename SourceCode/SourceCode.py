...
def checkIfInSquare(bottomLeft, topRight, point) :
   if (point[0] > bottomLeft[0] and point[0] < topRight[0] and point[1] > bottomLeft[1] and point[1] < topRight[1]) :
      return True
   else :
      return False
...
data = list(zip(cityX, cityY))  # Combine cityX and cityY into a list of tuples
kmeans = KMeans(n_clusters=2)  # Initialize KMeans with 2 clusters
kmeans.fit(data)  # Perform K-means clustering on the data
labels = kmeans.labels_  # Get the cluster labels for each data point
clusterCount = np.bincount(labels)  # Count the number of data points in each cluster


cluster_centers = kmeans.cluster_centers_  # Get the coordinates of the cluster centers
cs_x = cluster_centers[:, 0]  # Extract x-coordinates of cluster centers
cs_y = cluster_centers[:, 1]  # Extract y-coordinates of cluster centers


# Store the coordinates of cluster centers in the dictionary
